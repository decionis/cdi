...
import { StewardApiErrorMapper } from "./StewardApiErrorMapper";

async function mapped(error: unknown) {
  const response = StewardApiErrorMapper.toResponse(error);
  return { status: response.status, body: await response.json() };
}

describe("StewardApiErrorMapper", () => {
  it("maps an unauthorized error to 401", async () => {
    const { status, body } = await mapped(new StewardUnauthorizedError());
  ...
  it("maps a forbidden error to 403", async () => {
    const { status, body } = await mapped(new StewardForbiddenError());
  ...
  it("maps a not-found error to 404", async () => {
    const { status, body } = await mapped(new StewardNotFoundError("Account"));
  ...
  it("passes an upstream gateway status through", async () => {
    const { status, body } = await mapped(
      new StewardGatewayError("Upstream rejected the review", 409, {}),
    );
  ...
  it("clamps an out-of-range gateway status to 502", async () => {
    ...
    for (const upstreamStatus of [0, 199, 600, 1000]) {
      const { status } = await mapped(
        new StewardGatewayError("odd status", upstreamStatus, {}),
      );
  ...
  it("maps an unrecognized error to 500 without leaking its message", async () => {
    ...
    expect(body.message).toBe("Steward could not complete the request");
