    ...
    for (let attempt = 1; attempt <= this.maxAttempts; attempt += 1) {
      try {
        ...
        if (error instanceof GitHubApiError) {
          if (!this.isRetryable(error.status) || attempt === this.maxAttempts)
            throw error;
...
export class PullRequestBot {
  constructor({ api, repository, expectedAuthorLogin, defaultBranch }) {
    ...
    if (!expectedAuthorLogin)
      throw new Error("EXPECTED_AUTHOR_LOGIN is required");
  ...
  async run(eventName, event) {
    ...
    const creationRunId =
      eventName === "workflow_dispatch"
        ? this.readInput(event, "creation_run_id")
        : undefined;
    const branches = requestedBranch
      ? [requestedBranch]
      : await this.listCandidateBranches();
    const outcomes = [];
  ...
  async listCandidateBranches() {
    const branches = await this.api.paginate(
      "/repos/" + this.repository + "/branches",
    );
    return branches
      .map(({ name }) => name)
      .filter(
        (name) => typeof name === "string" && name !== this.defaultBranch,
      );
  }
  ...
  async ensurePullRequest(branch, creationRunId) {
    ...
    if (
      ...
      return {
        branch,
        status: "skipped",
        reason: "incomplete-commit-comparison",
      };
    }
    ...
    const commitsByExpectedAuthor = comparison.commits.every(
      (commit) =>
        commit.author?.login?.toLowerCase() === this.expectedAuthorLogin,
    );
    ...
    const pullRequest = await this.api.request(
      "POST",
  ...
  async hasTrustedCreationRun(branch) {
    const response = await this.api.request(
      "GET",
      "/repos/" + this.repository + "/actions/runs",
      {
        query: {
          event: "create",
          branch,
...
if (
  ...
  main().catch((error) => {
    process.stderr.write(
      (error instanceof Error ? error.message : "PR bot failed") + "\n",
    );
    process.exitCode = 1;
  });
}
