...
- [x] **`code_scanning` and `code_coverage` now have signals to consume.** Both rules used to wait on
      data this repository never produced. Code Security is enabled and CodeQL is publishing analyses,
      so `continue-on-error` is gone from `codeql.yml` and a real analysis failure blocks again.
      Coverage is generated by `pnpm test:coverage` with an 80% floor enforced in `vitest.config.ts`,
      matching the number the rule already demanded — measured at 92% statements and 88% functions
      when this landed.

      One step remains for you: add `analyze` to the required status checks so CodeQL blocks a merge
      rather than merely reporting. The command is in step 5 below.
