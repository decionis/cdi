...
The same App installation covers AgentSafe and Steward, but every repository
requires its own trusted workflow, variables, and encrypted secret. A token
minted here is scoped to `decionis/steward` and cannot access AgentSafe.
